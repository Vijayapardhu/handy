...
import 'data/background_codeforge.dart';
...
/// This build's version, read from the package at startup.
///
/// Shown in About and attached to every piece of feedback, which is why it was
/// worth taking out of a constant: it read "0.1.0" while the app was at 1.0.4,
/// so every bug report arrived tagged with a version that had never shipped and
/// nobody could reproduce against. A number that has to be edited by hand next
/// to one that is generated by the build is a number that will be wrong.
///
/// Includes the build number — two releases can share a version name, and the
/// build is what tells them apart in a report.
String handyVersion = '';
...
Future<void> main() async {
  ...
  // The daily CodeForge refresh runs through here: a launcher-side alarm fires
  // HomeWidgetBackgroundIntent, home_widget spins up a background isolate, and
  // this is the entrypoint it calls. See background_codeforge.dart and
  // WidgetTick.armDailyRefresh.
  HomeWidget.registerInteractivityCallback(codeForgeBackgroundCallback);
  ...
  await settings.load();
  // A local read out of the APK's own manifest, so it costs nothing worth
  // deferring — and everything that displays it can then be synchronous.
  final package = await PackageInfo.fromPlatform();
  handyVersion = package.buildNumber.isEmpty
      ? package.version
